...
import {getUserSubscribes} from "~/lib/api";
...
export default function FollowingPage() {
    ...
    const {
        ...
        queryFn: ({pageParam}) => getUserSubscribes(username, {nextPageToken: pageParam}),
    ...
    const users = data?.pages.flatMap((page) => page.subscribes) ?? [];

    return (
        <div className="flex min-h-screen flex-col gap-5 pb-20 mt-6">
            <title>Подписки | ТОЛК</title>
            <meta name="description" content="Список подписок пользователя." />
            <div className="sticky top-14 z-40 -mx-3 flex items-center gap-3 border-b bg-background/95 px-3 py-3 backdrop-blur sm:-mx-4 sm:px-4 md:-mx-8 md:px-8">
                ...
                <div className="min-w-0">
                    <h1 className="text-xl font-black leading-tight">Подписки</h1>
